var vue = new Vue({
  ...
  filters: {
    roundNumber: function(num){
      ...
      lastNum = num.substr(num.lastIndexOf('.')+1);
      if (lastNum.length == 3 && lastNum.substr(-1, 1) != 0) {
        num = parseFloat(num) + 0.01;
      }
      num = num + '';
      return parseFloat(num.substr(0, num.lastIndexOf('.') + 3)).toFixed(2);
    }
  },
  methods: {
    ...
    selectChannel: function(item, index) {
      ...
      if (item.payNewVo[0].chId == 73) { // 支付宝扫码渠道
        this.isShowTip = true;
      } else {
    ...
    getPayChannelList: function() {
      ...
      $.getJSON(url, param, function(data){
        ...
        if (self.channelList.length > 0) {
          self.selectChannel(self.channelList[0], 0);
        }
      })
    },
    ...
    charge: function () {
      ...
      var confId = $('.charge-list-active').attr('data-id');
    ...
    chargeMoney: function () {
      var self = this;
      // 交易所2、3限制整数充值
      if (self.exchangeId == 2 || self.exchangeId == 3) {
        if (self.money != parseInt(self.money)) {
          layer.msg('充值金额必须为整数，请重新输入');
          return;
        }
      }
      // 充值渠道73. 限制
